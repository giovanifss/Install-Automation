...
[[ "$EUID" -ne 0 ]] && echo "arch-git.sh: Run as root" 1>&2 && exit 1
pgrep sshd &> /dev/null || echo "arch-git.sh: SSH must be installed and running for git server" 1>&2
...
until passwd git; do
  echo "arch-git.sh: Could not set git password" 1>&2
done
